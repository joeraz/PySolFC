...
When adding new games, priority should be given to games that fit well within the current game engines, current cardset types, and the overall project scope and design philosophy.  While minor game variants are acceptable, not every possible variant is required.  Creating new cardset types is a major change, and should be reserved for major releases.
...
- Impractical does not mean forbidden.  Some ideas may be considered impractical given current architecture or resources, but PRs implementing them may still be accepted if they are clean, correct, and well-considered.  Such changes are reviewed with additional scrutiny due to their inherent risk.
